...
bit DelayWash(unsigned long TimeWash, signed int RDel1, int RInt) //---Таймер для стирок--- В минутах
{
...
bit FillWater(int Level, int PreW) //---Заливка воды--- Level = 1 или 2 --- PREW Залив воды I или II отделение.
{
	IntRelay = 1;
...
bit PWash(unsigned long TimePWash, int RelayWash, int RelayOsc, int PreW) //--- Процедура стирки --- Миллисекунды
{
	if (relayStart == 1)
	{
		if (RDel1==0)
...
void main()
{
	...
	*/
	...
	while(1)
	{
		...
		//void TempReg() //--- РЕГУЛИРОВКА ТЕМПЕРАТУРЫ ---
		{
		...
		//void ControlWater() //--- Доливка воды ---
		{
			if (IntRelay == 0)
			{
				LATE1 = 0; //Откл клапан.
